...
async def apply_fx_cache_and_billable_columns_patch(conn: AsyncConnection) -> None:
    """Кеш курсов ЦБ + billable в валюте проекта на записи времени."""
    await conn.execute(
        text(
            """
            CREATE TABLE IF NOT EXISTS time_tracking_fx_rate_cache (
                currency_code VARCHAR(10) NOT NULL,
                rate_date DATE NOT NULL,
                source VARCHAR(32) NOT NULL DEFAULT 'CBU_RU_UZ',
                uzs_per_unit NUMERIC(24, 10) NOT NULL,
    ...
    await conn.execute(
        text(
            "CREATE INDEX IF NOT EXISTS ix_tt_fx_cache_date ON time_tracking_fx_rate_cache (rate_date)"
        )
    )
    await add_columns_if_missing(
        conn,
        ...
        (
            ...
            "billable_currency VARCHAR(10)",
            "rate_source_amount NUMERIC(20, 6)",
            "rate_source_currency VARCHAR(10)",
            "fx_cross_rate NUMERIC(24, 12)",
            "fx_rate_date DATE",
            "fx_rate_source VARCHAR(32)",
            "billable_fx_as_of DATE",
        ),
    )
