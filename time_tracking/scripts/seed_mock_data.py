...
Почасовые ставки: сначала **обязательно** для каждой валюты созданных мок-проектов (billable + cost),
затем — остальные валюты из MOCK_CURRENCIES, если ещё нет (повторный запуск не дублирует).
...
async def _ensure_user_rates_for_project_currencies(
    session,
    users: list,
    projects_meta: list[tuple[str, str]],
    rng: random.Random,
) -> int:
    """Для каждой валюты из созданных мок-проектов — billable + cost, если ещё нет.

    Без ставок в **валюте проекта** дашборд даёт 0 по billable amount и по internal cost.
    """
    req = {_norm_rate_currency(c) for _, c in projects_meta if c}
    if not req:
        return 0
    ...
    for u in users:
        aid = u.auth_user_id
        for cur in sorted(req):
            for kind in ("billable", "cost"):
                ex = await hrr.list_by_user_and_kind(aid, kind)
                if any(_norm_rate_currency(r.currency) == cur for r in ex):
                    continue
                if kind == "billable":
                    amt = _random_billable_hourly(cur, rng)
                else:
                    br = await hrr.list_by_user_and_kind(aid, "billable")
                    base = next((x for x in br if _norm_rate_currency(x.currency) == cur), None)
                    if base is not None:
                        amt = (base.amount * Decimal(rng.randint(50, 75) / 100)).quantize(
...
async def _seed(
    *,
    ...
    async with async_session_factory() as session:
        ...
        n_proj_rates = await _ensure_user_rates_for_project_currencies(session, users, projects_meta, rng)
        await session.flush()
        ...
        n_rate_rows = n_proj_rates + await _seed_hourly_rates_if_missing(session, users, rng)
