...
async def _append_time_line(
    ...
    # В счёте количество часов и сумма считаются по фактическим часам (duration уже кратен минуте).
    qty = dec(entry.hours)
    ...
    *,
    ...
    for e in entries:
        if e.id in invoiced:
            continue
        h = dec(e.hours)
        amt, cur = _billable_amount_for_entry(h, True, e.work_date, rates.get(e.auth_user_id))
        out.append(
            {
                "id": e.id,
