...
async def update_workday_settings(
    ...
    """Прокси к обновлению настроек рабочего дня. Требует роль администратора."""
    user = _
    role = (user.get("role") or "").strip()
    if role not in ("Главный администратор", "Администратор", "Партнер"):
        raise HTTPException(status_code=403, detail="Only administrators can update workday settings")
    settings = get_settings()
    base = (settings.attendance_service_url or "").rstrip("/")
    if not base:
        raise HTTPException(status_code=503, detail="Attendance service not configured")
