...
**Фронт (SPA):** поле нормализуется в `expenseAuthor.ts` / `coerceExpense.ts`; отображается в таблице и карточках списка, в боковой панели заявки и в Excel-отчёте (репозиторий `tickets-front`).
