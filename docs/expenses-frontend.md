...
Микросервис **`expenses`** может отправлять HTML-письма на адреса **`EXPENSE_NOTIFY_TO`** (SMTP, см. `.env.example`):

1. **`POST /api/v1/expenses`** — уведомление о **черновике** (тема «… — черновик»), кнопка «Открыть заявку». Управляется **`EXPENSE_NOTIFY_ON_CREATE`** (по умолчанию `true`).
2. **`POST /api/v1/expenses/{id}/submit`** — уведомление о **поступлении на согласование** (тема «… — на согласование»), кнопки «Открыть», «Утвердить», «Отклонить». Управляется **`EXPENSE_NOTIFY_ON_SUBMIT`** (по умолчанию `true`).

Чтобы не получать два письма на одну заявку, задайте **`EXPENSE_NOTIFY_ON_CREATE=false`** и оставьте только письмо при submit.
