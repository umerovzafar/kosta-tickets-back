...
0. [Чеклист: что сделать на фронте](#0-чеклист-что-сделать-на-фронте)  
...
### 0.8 Не падать при 503 и «левые» ошибки в консоли

1. **Не обращайтесь к данным ответа**, пока не проверили **`response.ok`** (или `status === 200`). Иначе после **503** парсинг JSON или разбор `data.columns` даст **`null`** и падение вроде **`Cannot read properties of null (reading 'target')`** в вашем обработчике (если там обращение к `event.target` без проверки).

2. Ошибка в файле **`content.js`** в консоли часто идёт от **расширения браузера** (не из вашего бандла). Проверьте в режиме инкогнито без расширений или колонку **Stack** — если путь начинается с `chrome-extension://`, это не баг приложения.

3. Пример безопасной загрузки доски:

```ts
const res = await apiFetch('/api/v1/todos/board')
if (!res.ok) {
  if (res.status === 503) {
    // показать «Сервис временно недоступен»; при необходимости логировать await res.json() — там может быть hint
  }
  return null
}
const board = await res.json()
// дальше board.columns и т.д.
```
